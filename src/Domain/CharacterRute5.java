...
public class CharacterRute5 extends Character {
    ...
    public CharacterRute5(int x, int y, int imgNum) throws FileNotFoundException, IOException, InterruptedException {
        super(x, y, imgNum);
        setSprite();
        this.x = x;
        this.y = y;
    ...
    public void setSprite() throws FileNotFoundException, IOException {
        ...
        for (int i = 0; i <= 9; i++) {
            sprite.add(imageIO.read(new FileInputStream("src/Assets/SpritesPacman/" + i + ".png")));
        }
        for (int i = 0; i <= 9; i++) {
            sprite.add(imageIO.read(new FileInputStream("src/Assets/SpritesPacman/" + i + "R" + ".png")));
        }
                ...
                try {
                    if (!MainWindow.getInstance().isReverseRute()) {
                        ...
                        for (i = i; i <= this.PositionX.size() - 1; i++) {
                            System.out.println("X: " + PositionX.size());
                            System.out.println("Y: " + PositionY.size());
                            Thread.sleep(speed);
